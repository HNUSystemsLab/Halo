...
template <typename Key, typename T, typename Hash = std::hash<Key>,
          typename KeyEqual = std::equal_to<Key>, size_t HashPower = 21>
class clevel_hash {
 public:
  using key_type = Key;
  using mapped_type = T;
  ...
  clevel_hash() : meta(make_persistent<level_meta>().raw().off), thread_num(0) {
    ...
    int hp = hashpower;
    tmp->buckets = make_persistent<bucket[]>(pow(2, hp));
    tmp->capacity = pow(2, hp);
    tmp->up = nullptr;
    m->first_level.off = tmp.raw().off;

    tmp = make_persistent<level_bucket>();
    tmp->buckets = make_persistent<bucket[]>(pow(2, hp - 1));
    tmp->capacity = pow(2, hp - 1);
...
clevel_hash<Key, T, Hash, KeyEqual, HashPower>::search(
  ...
  hv_type hv = hash_func(key, sizeof(key));
    ...
    do {
      ...
      for (size_type j = 0; j < assoc_num; j++) {
        if (f_b.slots[j].x.partial == partial &&
          ...
          if (f_b.slots[j].p.get_address(my_pool_uuid)->first == key) {
      ...
      for (size_type j = 0; j < assoc_num; j++) {
        if (s_b.slots[j].x.partial == partial &&
          ...
          if (s_b.slots[j].p.get_address(my_pool_uuid)->first, key) {
...
clevel_hash<Key, T, Hash, KeyEqual, HashPower>::generic_insert(
  ...
  RETRY_INSERT:
    ...
    if (result == FOUND_IN_LEFT || result == FOUND_IN_RIGHT) {
      ...
      if (CAS1(&(e->off), old_e.raw(), created.p.raw())) {
    ...
    do {
      ...
      for (size_type j = 0; j < assoc_num; j++) {
        ...
        if (tmp.x.partial == partial && tmp.p.get_offset() != 0) {
          if ((tmp.p.get_address(my_pool_uuid)->first == key)) {
            if (CAS1(&(f_b.slots[j].p.off), tmp.p.off, 0)) {
              ...
              // before deletion's CAS1. Therefore, we can do context
      ...
      for (size_type j = 0; j < assoc_num; j++) {
        ...
        if (tmp.x.partial == partial && tmp.p.get_offset() != 0) {
          if ((tmp.p.get_address(my_pool_uuid)->first == key)) {
            if (CAS1(&(s_b.slots[j].p.off), tmp.p.off, 0)) {
  ...
  while (true) {
    ...
    if (result == FOUND_IN_LEFT || result == FOUND_IN_RIGHT) {
      if (succ_update && old_e == created.p) {
        ...
        return ret(true);
      } else if (CAS1(&(e->off), old_e.raw(), created.p.raw())) {
        pop.persist(&(e->off), sizeof(uint64_t));
        ...
        // find and before update's CAS1. Therefore, we can do
    ...
    while (true) {
      ...
      if (CAS1(&(meta.off), m_copy.off, tmp_meta[t_id].raw().off)) {
        ...
        if (cl->capacity >= new_capacity && m->is_resizing) {
          // CAS1 fails because other threads help updating meta
          delete_persistent_atomic<level_meta>(tmp_meta[t_id]);
          break;
        }
        // CAS1 fails because other threads complete rehashing.
      }
    }
  } else {
    // Ohter threads finished expanding
  ...
  while (run_expand_thread.get_ro().load()) {
    ...
    RETRY_REHASH:
      ...
      for (size_type slot_idx = 0; slot_idx < assoc_num; slot_idx++) {
        ...
        difference_type f_idx, s_idx;
        bool succ = false;
        hv_type hv = hash_func(e->first, sizeof(e->first));
        partial_t partial = get_partial(hv);
        ...
        for (size_type j = 0; j < assoc_num; j++) {
          ...
          if (dst_tmp.get_offset() == 0) {
            if (CAS1(&(dst_b1.slots[j].p.off), dst_tmp.raw(), src_tmp.raw())) {
          ...
          if (dst_tmp.get_offset() == 0) {
            if (CAS1(&(dst_b2.slots[j].p.off), dst_tmp.raw(), src_tmp.raw())) {
